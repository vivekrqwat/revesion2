    ...
    <Card className="bg-card border-border w-full">
      <CardHeader className="pb-3">
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-2">
          <CardTitle className="text-sm sm:text-base">Contribution Graph</CardTitle>
      ...
      <CardContent className="pb-4 overflow-hidden">
        <div className="w-full">
          <div
            className="grid gap-3 sm:gap-4 md:gap-6 p-2"
            style={{
          ...
          >
            {months.map((month, index) => (
              <div key={index} className="flex flex-col items-center gap-2">
                {/* Month Label */}
                <div className="text-xs font-semibold text-muted-foreground text-center h-6 flex items-center">
                  {month.name.substring(0, 3)}
                </div>

                {/* Grid */}
                <div
                  className="gap-0.5 flex flex-wrap justify-center"
                >
          ...
          {/* Legend */}
          <div className="flex items-center justify-center gap-2 sm:gap-3 mt-4 pt-3 border-t border-border text-xs text-muted-foreground flex-wrap">
            <span className="font-semibold">Activity:</span>
            <div className="flex items-center gap-2">
              <div className="h-2 w-2 sm:h-3 sm:w-3 rounded-sm bg-muted" />
              <span>No activity</span>
            </div>
            <div className="flex items-center gap-2">
              <div className="h-2 w-2 sm:h-3 sm:w-3 rounded-sm bg-green-500" />
              <span>Active</span>
            </div>
          </div>
        </div>
