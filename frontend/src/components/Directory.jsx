...
export default function Directory() {
  ...
  //   }
  // };
 const togglePublicPrivate = useCallback(async (dirId, currentStatus) => {
    try {
      setUpdatingDirId(dirId);
      const newStatus = !currentStatus;
      const response = await axios.put(
 ...
 const handleDeleteNoteConfirm = useCallback((noteId, dirId, noteName) => {
    setDeleteConfirm({ type: "note", id: noteId, dirId, itemName: noteName });
  }, []);
 ...
 const handleDeleteDirConfirm = useCallback((dirId, dirName) => {
    setDeleteConfirm({ type: "directory", id: dirId, itemName: dirName });
  }, []);
 ...
 const setNotes = useCallback(
  ...
  const getGradeColorClass = useCallback((grade) => {
    const colors = {
      green: "bg-green-500",
      red: "bg-red-500",
      yellow: "bg-yellow-400",
  ...
  // };
const getGradeBadgeClass = useCallback((grade) => {
    const colors = {
      green: "bg-green-500/20 text-green-700 border-green-500/30",
      red: "bg-red-500/20 text-red-700 border-red-500/30",
      yellow: "bg-yellow-500/20 text-yellow-700 border-yellow-500/30",
...
<GetALLDir
  ...
  updatingDirId={updatingDirId}
  getnotes={getnotes}
  togglePublicPrivate={togglePublicPrivate}
  openDir={openDir}
  notesMap={notesMap}
  handleDeleteDirConfirm={handleDeleteDirConfirm}
  handleDeleteNoteConfirm={handleDeleteNoteConfirm}
